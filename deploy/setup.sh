...
# 部署到 $HOME/opt/authcenter，以當前使用者身份執行（不需要 sudo）
set -e

APP_DIR="${AUTHCENTER_DEPLOY_DIR:-$HOME/opt/authcenter}"
...
# 動態替換 service 檔中的路徑
sed "s|/home/kane.beh/opt/authcenter|$APP_DIR|g; s|/home/kane.beh|$HOME|g" \
    "$APP_DIR/deploy/authcenter.service" > "$HOME/.config/systemd/user/authcenter.service"
